...
def train_model_ac(name,base_save_path):
    ...
    model = TD3(
        policy="MlpPolicy",
        env=env,
        learning_rate=linear_schedule(1e-5), #嘗試學習率隨時間遞減或者設定得更小
        learning_starts=6000,
        buffer_size=70000, #大一點比較好
        batch_size=1024, 
        tau=0.01,
        gamma=0.99,
        seed=26,
        ...
        target_policy_noise=0.2,  # 添加TD3特有的目標策略噪聲
        target_noise_clip=0.5,  # 添加TD3特有的噪聲剪裁
        policy_delay=2  # 更新策略的延遲步驟
    )
    # 訓練模型
    ...
    print("td3_ac_temp_humi_co2訓練完成")


def train_model_win(name,base_save_path):
    log_dir = os.path.join(base_save_path)
    replay_buffer_path = os.path.join(base_save_path, str(name+"_buffer.pkl"))
    model_dir=os.path.join(base_save_path, name)
    env_dir=os.path.join(base_save_path,str(name+"_env.pkl"))
    ...
    # 訓練模型
    model.learn(total_timesteps=10, log_interval=1, tb_log_name=tb_log_name,callback=custom_callback)

    # 保存模型
    env.save(env_dir)
    model.save(model_dir)
    model.save_replay_buffer(replay_buffer_path)
    print("td3_win_temp_humi_co2訓練完成")

def get_ac_temp_humi_buffer(name,base_save_path): #取得前一百萬筆的replay buffer
    # 加載模型
    model = TD3.load(os.path.join(base_save_path, name))
    ...
    if buffer_size > 0:
        ...
        # 提取數據
        observations = model.replay_buffer.observations[:max_records, 0, :]  # 修改此行
...
if __name__ == "__main__":
    name="td3_ac_temp_humi_co2_50000" #指定儲存檔案名稱
    base_save_path = "G:\我的雲端硬碟\paper\code\DDPG_implementation\compare_model/td3_train_result" #指定要儲存的資料夾
